...
class neural_network:
    ...
    #n_hidden  _layers is a list of number of neurons at each level.
    def __init__(self, input_layer_n, n_hidden_layers, num_classes):
        ...
        for num_neurons in self.n_hidden_layers:
            #weights label contains list of weights and derivative contains derivative w.r.t aggregate input.
            self.hidden_layers.append([dict({'weights':[random.random() for x in range(prev_num_neurons+1)],'derivative':0}) for y in range(num_neurons)])
            prev_num_neurons = num_neurons
            
        self.output_layer = [dict({'weights':[random.random() for x in range(self.n_hidden_layers[-1]+1)],'derivative':0}) for y in range(num_classes)]
    ...
    def forward_propogate(self, x):
       ...
       for layer in self.hidden_layers:
           ...
           for neuron in range(len(layer)):
               #using sigmoid funtion as activation
    ...
    def back_propogate(self, ydata):
        #For output layer with softmax and cross-entropy
        for neuron, y in zip(self.output_layer,ydata):
            neuron['derivative']+= (neuron['value']-y)
        
        prev_layer = self.output_layer    
        for layer in reversed(self.hidden_layers):
            #Sum of derivatives times weights for above layer
            for neuron, neuron_index in zip(layer, range(len(layer))):
                ...
                #Multiply above sum by derivative of activation function (sigmoid)
    ...
    def update_weights(self, eta):
        ...
        #Derivative = error for input times input value times training error (= eta)
            ...
            for neuron in layer:
                if layer_below_index>=0:
                    for windex in range(len(neuron['weights'])-1):
                        neuron['weights'][windex] -= eta*(neuron['derivative'])*self.hidden_layers[layer_below_index][windex]['value']
                    neuron['weights'][-1] -= eta*(neuron['derivative'])
                #For bottom most hidden layer
                else:
                    for windex in range(len(neuron['weights'])-1):
                        neuron['weights'][windex] -= eta*(neuron['derivative'])*self.input_layer[windex]
                    neuron['weights'][-1] -= eta*(neuron['derivative'])
                neuron['derivative'] = 0
